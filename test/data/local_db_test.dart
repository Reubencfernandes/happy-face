...
void main() {
  ...
  test('search matches captions, tags, places, names and dates by prefix', () {
    put(
      rec(
        'beach',
        DateTime.utc(2025, 12, 25, 10),
        caption: 'Two kids building a sandcastle',
        tags: ['beach', 'sunset'],
        place: 'Panaji',
        country: 'India',
      ),
    ...
    expect(ids('beach 2025'), ['beach']);
    expect(ids('beach 2024'), isEmpty);
  ...
  test('job queue with retry delays', () {
    ...
    db.enqueueJob('x', JobKind.caption);
    db.enqueueJob('y', JobKind.caption);
    db.enqueueJob('y', JobKind.caption); // duplicates are ignored
    db.enqueueJob(
      'x',
      JobKind.weather,
    ...
    expect(db.dueJobs(JobKind.caption, now).map((j) => j.photoId), ['y', 'x']);
    expect(db.dueJobs(JobKind.weather, now), isEmpty);
    db.retryJobLater('y', JobKind.caption, now.add(const Duration(hours: 1)));
    expect(db.dueJobs(JobKind.caption, now).map((j) => j.photoId), ['x']);
    db.completeJob('x', JobKind.caption);
    expect(db.jobCount(JobKind.caption), 1);
    expect(
      db
          .dueJobs(JobKind.caption, now.add(const Duration(hours: 2)))
          .single
          .attempts,
      1,
    );
